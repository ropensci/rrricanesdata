...
#' ########################################################################## '#
## ---- Forecast/Advisory ------------------------------------------------------
fstadv <- read_csv("./datasets/fstadv.csv",
                   col_types = cols(),
                   guess_max = 13000
)
...
#' exist.
fstadv_wr <- fstadv %>%
  select(matches(".[NE|SE|SW|NW][34|50|64].")) %>%
  map_df(as.numeric)
